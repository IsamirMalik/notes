...
function minCost(costs) {
    ...
    let firstHalf = Math.floor(costs.length / 2);
    let finalCost = 0;

    for (let index = 0; index < firstHalf; index++) {
        finalCost += costs[index][0];
        finalCost += costs[index + firstHalf][1];
    }

    // for (let index = firstHalf; index < costs.length; index++) {
    //     finalCost += costs[index][1];
    // }
